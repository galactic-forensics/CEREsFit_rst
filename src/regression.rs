...
pub fn linear_fit(data: &Data) -> Result<LinearFit, String> {
    ...
    if let Some(ref rho) = data.rho {
        if rho.len() != len_xdat {
            return Err("Length of rho does not match xdat".to_owned());
        }
    }

    if let Some(ref fixpt) = data.fixpt {
        if fixpt.len() != 2 {
    ...
    let sigxy = match &data.rho {
        Some(rho) => rho * &data.sigx * &data.sigy,
        None => Array1::zeros(data.sigx.len()),
    };

    // Calculate all quantities for the linear fit
    let (slope, xbar, ybar, weights) = calculate_slope(data, &sigxy)?;
    let intercept = ybar - slope * xbar;
    let mswd = calculate_mswd(
        &data.xdat,
        &data.ydat,
        slope,
        intercept,
    ...
    let [slope_unc, intercept_unc] = match &data.fixpt {
        Some(fixpt) => unc_calc_fixpt(data, &sigxy, slope, xbar, ybar, &weights, fixpt),
        None => unc_calc_no_fixpt(data, &sigxy, slope, xbar, ybar, &weights),
    };
...
fn calculate_mswd(
    ...
    let chi_square = (weights * (ydat - slope * xdat - intercept).mapv(|x: f64| x.powi(2))).sum();
    let dof = match fixpt {
        Some(_) => xdat.len() - 1,
        None => xdat.len() - 2,
    };
...
fn calculate_slope(
    ...
    let sigx_sq = data.sigx.mapv(|x: f64| x.powi(2));
    let sigy_sq = data.sigy.mapv(|x: f64| x.powi(2));

    let calc_xbar = |weights: &Array1<f64>| -> f64 {
        match &data.fixpt {
            Some(f) => f[0],
            None => (weights * &data.xdat).sum() / weights.sum(),
        }
    };
    ...
    let calc_ybar = |weights: &Array1<f64>| -> f64 {
        match &data.fixpt {
            Some(f) => f[1],
            None => (weights * &data.ydat).sum() / weights.sum(),
        }
    };

    let calc_weights =
        |slp: f64| -> Array1<f64> { 1.0 / (&sigy_sq + slp.powi(2) * &sigx_sq - 2.0 * slp * sigxy) };

    let iterate_slope = |slp: f64| -> f64 {
        let weights = calc_weights(slp);
        let weights_sq = &weights * &weights;
        let u_all = &data.xdat - calc_xbar(&weights);
        let v_all = &data.ydat - calc_ybar(&weights);
    ...
    let mut slope_old = initial_guesses_slope(&data.xdat, &data.ydat)?;
...
fn unc_calc_fixpt(
    ...
    let u_all = &data.xdat - xbar;
    let v_all = &data.ydat - ybar;
    ...
    for (it, sigxi) in data.sigx.indexed_iter() {
        let sigyi = data.sigy[it];
        let sigxyi = sigxy[it];
